...
- Resources -> Proxies:  **Manual proxy configuration**，设置代理

  Web Server(HTTP):  http://127.0.0.1:7890

  Secure Web Server(HTTPS): http://127.0.0.1:7890

  Bypass proxy settings for these hosts & domains:   localhost,127.0.0.1,::1

- Resources -> Network: **Enable host networking**

  在 Linux 原生 Docker 中，当你运行容器时加上 `--net=host` 参数：`docker run --net=host my-app`

  容器会**直接共享宿主机的网络命名空间**，也就是说：

  - 容器内监听的端口（如 `8080`）会直接绑定到 **宿主机的 `localhost:8080`**
  - 容器可以直接通过 `localhost` 访问宿主机上运行的服务（如数据库、API 等）

  这比 `-p 8080:8080` 更高效（无 NAT 开销），常用于性能敏感或需要广播/多播的场景。

  **✅ “Enable host networking” 选项的作用**

  当你在 Docker Desktop 设置中 **启用此选项**（Settings > General > ☑ Enable host networking）：

  > Docker Desktop 会建立 **从 Windows 主机到 WSL 2 中 host network 容器的自动端口转发**。

  效果：

  - 容器以 `--net=host` 运行并监听 `:3000`
  - 你可以在 **Windows 浏览器**中直接访问 `http://localhost:3000`
  - 同样，容器也可以通过 `localhost` 访问 Windows 上运行的服务（如 `localhost:1433` 的 SQL Server）

- Resources -> Network: **Default networking node** : IPv4 only    只使用ipv4
...
- Resources -> WSL Integration: **Enable integration with my default WSL distro** is enabled.
  ...
  **是否勾选子系统说明：**
  ...
  $ wsl -l -v
    NAME              STATE      VERSION
  * Ubuntu-22.04      Running    2     ← 默认发行版
    Ubuntu2404        Stopped    2
  ```

  - 如果 **只勾选** “Enable integration with my default WSL distro”：
    - 你只能在 `Ubuntu-22.04` 里用 `docker` 命令。
    - 在 `Ubuntu2404` 里输入 `docker` 会提示命令未找到，或无法连接 daemon。
  - 如果 **额外勾选 `Ubuntu2404`**：
    - 两个发行版都能无缝使用 `docker`，且共享所有 Docker 资源。

  **总结**

  | 选项                                                         | 作用                                          |
  | ------------------------------------------------------------ | --------------------------------------------- |
  | **Enable integration with my default WSL distro**            | 让系统默认的 WSL 发行版能用 Docker            |
  | **Enable integration with additional distros: [✓ Ubuntu2404]** | 让 `Ubuntu2404` 这个非默认发行版也能用 Docker |

  ✅ **建议**：如果你经常在 `Ubuntu2404` 里开发，就勾选它，这样就能直接使用 `docker`、`docker-compose` 等命令，体验和原生 Linux 几乎一致。
